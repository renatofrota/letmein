...
rm -f "$0"
echo -e "\n\tLetmein - v1.0.6 - https://github.com/renatofrota/letmein"
...
EOF
    ...
    JSON_FILE1=~/tmp/skipdns/$(echo "$SITE_URL" | sed 's|//www.|//|' | sed 's|https://||' | sed 's|http://||' | sed 's|/||')
    JSON_FILE2=/tmp/skipdns/$(echo "$SITE_URL" | sed 's|//www.|//|' | sed 's|https://||' | sed 's|http://||' | sed 's|/||')
